...
namespace Microsoft.PackageManagement.PackageSourceListProvider
{
    ...
    public abstract class PackageSourceListRequest : Request {
        ...
        private const string PowerShellSourceURL = @"http://go.microsoft.com/fwlink/?LinkID=821777&clcid=0x409";
        private const string PowerShellNanoSourceURL = @"http://go.microsoft.com/fwlink/?LinkID=821783&clcid=0x409";
        private const string PowerShellSourceCatalogURL = @"http://go.microsoft.com/fwlink/?LinkID=823093&clcid=0x409";
        private const string PowerShellNanoSourceCatalogURL = @"http://go.microsoft.com/fwlink/?LinkID=823094&clcid=0x409";
        ...
        internal static readonly Regex RegexFastPathComplex = new Regex(@"\$(?<location>[\w,\+,\/,=]*)#(?<id>[\w,\+,\/,=]*)#(?<displayname>[\w,\+,\/,=]*)#(?<version>[\w,\+,\/,=]*)#(?<fastpath>[\w,\+,\/,=]*)#(?<providername>[\w,\+,\/,=]*)");

        internal readonly Lazy<bool> AllVersions;
        internal readonly Lazy<string[]> Headers;
        internal Lazy<bool> SkipHashValidation;
        internal Lazy<bool> AddToPath;
        internal Lazy<bool> RemoveFromPath;
        internal readonly Lazy<string> Scope;

        internal static string AppdataFolder = Environment.GetEnvironmentVariable("appdata");
...
<configuration>
  <packageSources>    
        ...
        {           
            ...
            AddToPath = new Lazy<bool>(() => GetOptionValue("AddToPath").IsTrue());
            RemoveFromPath = new Lazy<bool>(() => GetOptionValue("RemoveFromPath").IsTrue());
        ...
        internal string DefaultJSONFileLocation
        {
            get
            {
                return Path.Combine(AppdataFolder, Constants.ProviderName, Constants.JSONFileName);
            }
        }
        ...
        internal string DefaultCatlogFileLocation
        {
            get
            {
                return Path.Combine(AppdataFolder, Constants.ProviderName, Constants.CatFileName);
            }
        }
        ...
        /// </summary>
        [SuppressMessage("Microsoft.Performance", "CA1811: Avoid uncalled private coded", Justification = "Duck type needs it.")]
        ...
        internal static string MakeFastPathComplex(string source, string name, string displayName, string version, string fastPackageReference, string providerName)
        {
            return string.Format(CultureInfo.InvariantCulture, @"${0}#{1}#{2}#{3}#{4}#{5}", (source ?? "").ToBase64(), (name ?? "").ToBase64(), (displayName ?? "").ToBase64(), (version ?? "").ToBase64(), (fastPackageReference ?? "").ToBase64(), (providerName ?? "").ToBase64());
        }

        internal bool TryParseFastPathComplex(string fastPath, Regex regex, out string location, out string id, out string displayname, out string version, out string fastpath, out string providerName)
        {
            var match = regex.Match(fastPath);
            ...
            providerName = match.Success ? match.Groups["providername"].Value.FromBase64() : null;

            return match.Success;
        }

        internal string GetProviderNameFromFastPathComplex(string fastPath)
        {
            ...
            if (!TryParseFastPathComplex(fastPath: fastPath, regex: PackageSourceListRequest.RegexFastPathComplex, location: out sourceLocation, id: out id, displayname: out displayname, version: out version, fastpath: out fastPackageReference, providerName: out providerName))
            {
                return "";
            }
            return providerName;
        ...
        internal PackageJson GetFastReferenceComplex(string fastPath)
        {
            ...
            if (TryParseFastPathComplex(fastPath: fastPath, regex: RegexFastPathComplex, location: out sourceLocation, id: out id, displayname: out displayname, version: out version, fastpath: out fastreference, providerName: out providerName))
            {               
        ...
        internal PackageJson GetPackageFromFastPath(string fastPath)
        {
            string sourceLocation;
            string id;
            string displayname;
        ...
        internal IEnumerable<PackageSource> SelectedSources
        {
            get
            {
                ...
                foreach (var src in sourcesFromUser)
                {
                    ...
                    if (File.Exists(src) && (Path.GetExtension(src).EqualsIgnoreCase(".json")) )
                    {                     
                    ...
                    else
                    {
                        ...
                        {
                            Verbose(Resources.Messages.UnsupportedPackageSource, Constants.ProviderName, src);
                        }
                        else
                        {
                            Warning(Resources.Messages.UnsupportedPackageSource, Constants.ProviderName, src);                            
                        }
                    }
                }
            }
        ...
        internal void PromptAndDownloadSampleJSON()
        {
            ...
            if (force || this.ShouldContinue(Resources.Messages.QueryDownloadPackageSourceList.format(DefaultJSONSourceLocation), Resources.Messages.PackageSourceListNotFound.format(DefaultJSONFileLocation)))
            {
                ...
                {
                    this.AddPackageSource("PSL", DefaultJSONFileLocation, false, false);
                }
            }
        }
