...
namespace Microsoft.PackageManagement.PackageSourceListProvider
{
    internal static class JsonParser
    {
        ...
        public static Dictionary<string, List<PackageJson>> ReadPackageSpec(PackageSource packageSource, string fileToBeProcessed)
        {
            ...
            var packageListEntries = PopulatePackageSourceList(result, packageSource);
        ...
        public static Dictionary<string, List<PackageJson>> PopulatePackageSourceList(PSObject psObjectEntries, PackageSource packageSource)
        {
                ...
                else if (package.Value is PSObject)
                {
                    ...
                    if (convertedPackage != null)
                    {
                        ...
                        else
                        {
                            ...
                            convertedPackage.Name = package.Name;

                            if (!EnsureRequiredFieldsExist(convertedPackage))
                            {
                        ...
                        if (Constants.MediaType.ExePackage.EqualsIgnoreCase(convertedPackage.Type) ||
                            Constants.MediaType.MsiPackage.EqualsIgnoreCase(convertedPackage.Type) ||
                            Constants.MediaType.MsuPackage.EqualsIgnoreCase(convertedPackage.Type))
                        {
                            continue;
                        }
                        else
        ...
        private static bool EnsureRequiredFieldsExist( PackageJson package)
        {
            if (!string.IsNullOrWhiteSpace(package.Version)
                && !string.IsNullOrWhiteSpace(package.Source)
                && !string.IsNullOrWhiteSpace(package.Type))
            {
                return true;
            }
        ...
        private static List<PackageJson> PopulateFromArray(object[] listOfPackages, string packageSpecPath, string key)
        {
            ...
            {
                throw new ArgumentNullException("listOfPackages");
            }
    ...
    public class PackageJson
    {
       ...
       [SuppressMessage("Microsoft.Naming", "CA1721:PropertyNamesShouldNotMatchGetMethod", Justification = "ISO field name.")]
        ...
        public string Destination
        {
            ...
            set { _destination = Environment.ExpandEnvironmentVariables(value); }
        }

        internal PackageSource PackageSource { get; set; }

        [SuppressMessage("Microsoft.Usage", "CA2227: Collection properties should be read only", Justification = "Json Serialize/Deserializer needs it.")]
